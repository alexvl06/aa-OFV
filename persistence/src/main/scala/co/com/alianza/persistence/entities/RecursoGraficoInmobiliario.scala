...
case class RecursoGraficoInmobiliarioTable(tag: Tag) extends Table[RecursoGraficoInmobiliario](tag, "RECURSO_GRAFICO_INMOBILIARIO") {
  ...
  def posicion = column[Option[Int]]("POSICION")

  def * = (id, titulo, url, visible, administrable, menuPrincipal, posicion) <> (RecursoGraficoInmobiliario.tupled, RecursoGraficoInmobiliario.unapply)
}
