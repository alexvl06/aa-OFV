...
import co.com.alianza.persistence.entities.{ UsuarioAgenteInmobiliario, UsuarioAgenteInmobiliarioTable, UsuarioEmpresarial, UsuarioEmpresarialTable }
 ...
 */
case class UsuarioAgenteInmobDAO(implicit dcConfig: DBConfig) extends TableQuery(new UsuarioAgenteInmobiliarioTable(_)) with UsuarioEmpresarialDAOs {

  import dcConfig.DB._
  import dcConfig.driver.api._

  def create(agenteEmpresarial: UsuarioAgenteInmobiliario): Future[Int] = {
  ...
  def update(id: Int, usuario: String, correo: String, nombreUsuario: String, cargo: String, descripcion: String): Future[Int] = {
    val query = this.filter(_.id === id).map(a => (a.correo, a.usuario, a.descripcion))
  ...
  def getById(idUsuario: Int): Future[Option[UsuarioAgenteInmobiliario]] = {
