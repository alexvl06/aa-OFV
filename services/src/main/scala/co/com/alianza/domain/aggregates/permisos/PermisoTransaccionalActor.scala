...
class PermisoTransaccionalActor extends Actor with ActorLogging with FutureResponse {
  ...
  def receive = {
    case GuardarPermisosAgenteMessage(idAgente, encargosPermisos) =>
      ...
      val permisos = encargosPermisos flatMap {e => e.permisos.map(p => p.copy(permiso = p.permiso.copy(idEncargo = e.wspf_plan, idAgente = idAgente)))}
    ...
    case (permisoAgentes: PermisoTransaccionalUsuarioEmpresarialAgentes, currentSender: ActorRef) =>
      val future = (for {
        result <- ValidationT(DataAccessAdapter guardaPermiso (permisoAgentes.permiso, permisoAgentes.idsAgentes))
      } yield result).run
