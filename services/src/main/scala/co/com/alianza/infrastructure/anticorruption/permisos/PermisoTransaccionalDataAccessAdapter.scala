...
object PermisoTransaccionalDataAccessAdapter {
  ...
  def consultaPermisosAgente(idAgente: Int): Future[Validation[PersistenceException, (List[Permiso], List[EncargoPermisos])]] = {
    ...
    repository.consultaPermisosAgente(idAgente).map {
      case zSuccess(
        p: (List[(co.com.alianza.persistence.entities.PermisoAgente, List[(Option[co.com.alianza.persistence.entities.PermisoAgenteAutorizador], Option[Boolean])])], List[(String, List[(co.com.alianza.persistence.entities.PermisoTransaccionalUsuarioEmpresarial, List[(Option[co.com.alianza.persistence.entities.PermisoTransaccionalUsuarioEmpresarialAutorizador], Option[Boolean])])])])) =>
