...
object PermisoTransaccionalDataAccessTranslator {
  ...
  type permisosGenerales = List[(String, List[(ePermisoTxUsuarioEmpresarial, List[(Option[ePermisoTxUsuarioEmpresarialAutorizador], Option[Boolean])])])]
  ...
  def aEncargoPermisosDTO(
    idEncargo: String,
    e: List[(ePermisoTxUsuarioEmpresarial, List[(Option[ePermisoTxUsuarioEmpresarialAutorizador], Option[Boolean])])]
  ): EncargoPermisos =
    EncargoPermisos(wspf_plan = idEncargo, permisos = e map aPermisoTransaccionalUsuarioEmpresarialAgentes)

  def aPermisoTransaccionalUsuarioEmpresarialAgentes(permiso: (ePermisoTxUsuarioEmpresarial, List[(Option[ePermisoTxUsuarioEmpresarialAutorizador], Option[Boolean])])): PermisoTransaccionalUsuarioEmpresarialAgentes = {
