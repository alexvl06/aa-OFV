...
object DataAccessTranslator {
  ...
  def translate(recursos:List[eRecursoPerfilClienteAdmin]) =
    recursos map (recurso => RecursoPerfilClienteAdmin(recurso.idPerfil, recurso.urlRecurso, recurso.acceso, recurso.filtro))
