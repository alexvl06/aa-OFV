...
case class AutenticacionComercialDriverRepository(alianzaLdapDAO: AlianzaLdapDAO) extends AutenticacionComercialRepository {
  ...
  override def authenticateLDAP(userType: Int, username: String, password: String, ip: String): Future[String] = ???

  override def authenticateAdmin(username: String, password: String, ip: String): Future[String] = ???
}
