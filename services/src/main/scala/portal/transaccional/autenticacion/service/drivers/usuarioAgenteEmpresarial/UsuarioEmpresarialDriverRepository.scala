...
case class UsuarioEmpresarialDriverRepository(usuarioDAO: UsuarioEmpresarialDAO)(implicit val ex: ExecutionContext)
   ...
   * @param contrasena
   */
  override def actualizarContrasena(idUsuario: Int, contrasena: String): Future[Int] = {
    for {
  ...
  override def validarUsuario(usuarioOption: Option[UsuarioAgente]): Future[UsuarioAgente] = {
    usuarioOption match {
